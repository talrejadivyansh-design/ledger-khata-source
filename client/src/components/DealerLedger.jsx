    ...
    <div className="space-y-4">
      <div className="rounded-lg border border-slate-200 bg-white p-3 shadow-sm">
        <div className="flex items-start justify-between">
          ...
          <button onClick={handleDeleteDealer} className="text-xs text-red-500">
            Delete dealer
          </button>
        </div>
        <p className="mt-2 text-sm">
          ...
          <span className={`font-semibold ${currentBalance > 0 ? "text-red-600" : "text-green-600"}`}>
            ₹{Math.abs(currentBalance).toLocaleString("en-IN")} {currentBalance > 0 ? "(owed)" : currentBalance < 0 ? "(advance)" : ""}
          </span>
        </p>
      </div>
